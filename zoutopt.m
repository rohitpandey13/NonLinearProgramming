function [xopt,fopt,iter] = zoutopt(funz,delf,delg1,delg2,x0,xl,xu,nc,ncs,crit,kmax)
% Inputs:
% funz: objective function and constraints
% delf: gradient of f
% delg1: gradient of g1
% delg2: gradient of g2
...
% Simpx routine is adopted from Dr.YKYeo
% Outputs:
% xopt: optimal point
...
while (1)
 iter = iter + 1;
 [nc, na] = actcont(ncs, crit, g1,g2); % active constraints
 if (iter > kmax) 
     disp('Maximim possible iterations exceeded.');
     break; 
 end
 [d, dn, beta] = dirvec(delf,delg1,delg2,crit,nv,x,nc,xl,xu,mcrit); 
...
function [nc, na] = actcont(ncs, mcrit, g1,g2)
for i = 1:ncs, na(i) = i; end
nc = 0;
for k = 1:ncs
 if ( g1(k) > -mcrit || g2(k) > -mcrit), nc = nc + 1; ntemp = na(k); na(k) = na(nc);
na(nc) = ntemp; end
end
end

function [d, dn, beta] = dirvec(delf,delg1,delg2,crit,nv,x,nc,xl,xu,mcrit)
...
x = x0 + xm*d; fg = funz(x); f = fg(1); g1 = fg(2); g2=fg(3); gmax = max(g1,g2); fm = f;
